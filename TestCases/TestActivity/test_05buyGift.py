...
exc = HandleExcel(datas_dir + "\\Activitys.xlsx", "买赠活动")
caseDatas = exc.read_all_datas()
exc.close_file()

@ddt
class TestbuyGift(unittest.TestCase,BaseTest):
    @classmethod
    def setUpClass(cls) -> None:
        logger.info("***************测试用例（买赠活动）开始执行***************")
        cls.Activity("buyGift")

    @classmethod
    def tearDownClass(cls) -> None:
        logger.info("***************测试用例（买赠活动）执行结束***************")

    @list_data(caseDatas)
    def test_buyGift(self,case):
