...
# --*-- coding: utf-8 --*--
# @Author  : dwf
# @Time    : 2022/7/05 11:01
# @File    : test_02specialPrice.py
...
exc = HandleExcel(datas_dir + "\\Activitys.xlsx", "限时促销")
caseDatas = exc.read_all_datas()
exc.close_file()

@ddt
class TestspecialPrice(unittest.TestCase,BaseTest):
    @classmethod
    def setUpClass(cls) -> None:
        logger.info("***************测试用例（限时促销）开始执行***************")
        cls.Activity("specialPrice")

    @classmethod
    def tearDownClass(cls) -> None:
        logger.info("***************测试用例（限时促销）执行结束***************")

    @list_data(caseDatas)
    def test_specialPrice(self,case):
