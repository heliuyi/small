...
exc = HandleExcel(datas_dir + "\\Activitys.xlsx", "部分满减")
caseDatas = exc.read_all_datas()
exc.close_file()

@ddt
class TestpartFullReduce(unittest.TestCase,BaseTest):
    @classmethod
    def setUpClass(cls) -> None:
        logger.info("***************测试用例（部分满减）开始执行***************")
        cls.Activity("partFullReduce")

    @classmethod
    def tearDownClass(cls) -> None:
        logger.info("***************测试用例（部分满减）执行结束***************")

    @list_data(caseDatas)
    def test_partFullReduce(self,case):
