...
exc = HandleExcel(datas_dir + "\\Activitys.xlsx", "预售活动")
caseDatas = exc.read_all_datas()
exc.close_file()

@ddt
class TestpreSale(unittest.TestCase,BaseTest):
    @classmethod
    def setUpClass(cls) -> None:
        logger.info("***************测试用例（预售活动）开始执行***************")
        cls.Activity("preSale")

    @classmethod
    def tearDownClass(cls) -> None:
        logger.info("***************测试用例（预售活动）执行结束***************")

    @list_data(caseDatas)
    def test_preSale(self,case):
