...
# @Author  : dwf
# @Time    : 2022/7/5 14:54
# @File    : test_04deliveryFullReduce.py
...
exc = HandleExcel(datas_dir + "\\Activitys.xlsx", "配送费满减")
caseDatas = exc.read_all_datas()
exc.close_file()

@ddt
class TestdeliveryFullReduce(unittest.TestCase,BaseTest):
    @classmethod
    def setUpClass(cls) -> None:
        logger.info("***************测试用例（配送费满减）开始执行***************")
        cls.Activity("deliveryFullReduce")

    @classmethod
    def tearDownClass(cls) -> None:
        logger.info("***************测试用例（配送费满减）执行结束***************")

    @list_data(caseDatas)
    def test_deliveryFullReduce(self,case):
