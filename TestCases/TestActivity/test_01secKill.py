...
# --*-- coding: utf-8 --*--
# @Author  : dwf
# @Time    : 2022/6/30 14:53
# @File    : test_01secKill.py
...
@ddt
class TestsecKill(unittest.TestCase,BaseTest):

    @classmethod
    def setUpClass(cls) -> None:
        logger.info("***************测试用例（秒杀活动）开始执行***************")
        cls.Activity("secKill")

    # def setUp(self) -> None:
    #     self.add_Seckillactivity()

    @classmethod
    def tearDownClass(cls) -> None:
        logger.info("***************测试用例（秒杀活动）执行结束***************")

    @list_data(caseDatas)
    def test_secKill(self,case):
