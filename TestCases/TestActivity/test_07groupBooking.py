...
exc = HandleExcel(datas_dir + "\\Activitys.xlsx", "拼团活动")
caseDatas = exc.read_all_datas()
exc.close_file()

@ddt
class TestgroupBooking(unittest.TestCase,BaseTest):
    @classmethod
    def setUpClass(cls) -> None:
        logger.info("***************测试用例（拼团活动）开始执行***************")
        cls.Activity("groupBooking")

    @classmethod
    def tearDownClass(cls) -> None:
        logger.info("***************测试用例（拼团活动）执行结束***************")

    @list_data(caseDatas)
    def test_groupBooking(self,case):
