...
exc = HandleExcel(datas_dir + "\\Activitys.xlsx", "社区团购")
caseDatas = exc.read_all_datas()
exc.close_file()

@ddt
class TestgroupCommunity(unittest.TestCase,BaseTest):
    @classmethod
    def setUpClass(cls) -> None:
        logger.info("***************测试用例（社区团购）开始执行***************")
        cls.Activity("groupCommunity")

    @classmethod
    def tearDownClass(cls) -> None:
        logger.info("***************测试用例（社区团购）执行结束***************")

    @list_data(caseDatas)
    def test_groupCommunity(self,case):
        print("字典:{}".format(type(case)))  ##字典
        print("字符串：{}".format(type(case["expected"])))  # 字符串
