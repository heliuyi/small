...
exc = HandleExcel(datas_dir + "\\Goods.xlsx", "商品分类")
caseDatas = exc.read_all_datas()
exc.close_file()

@ddt
class Testcategory(unittest.TestCase,BaseTest):
    @classmethod
    def setUpClass(cls) -> None:
        logger.info("***************测试用例（商品分类）开始执行***************")
    ...
    def tearDownClass(cls) -> None:
        logger.info("***************测试用例（商品分类）执行结束***************")

    @list_data(caseDatas)
    def test_category(self,case):
